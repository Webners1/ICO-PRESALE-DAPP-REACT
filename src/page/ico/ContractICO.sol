...
contract CryptosICO is Cryptos{
    ...
    uint public tokenPrice = tokenSupply/hardCap;  // 1 ETH = 1000 CRTP, 1 CRPT = 0.001
    ...
    uint public totalWallet=0;
    uint public balanceBNB;
    enum State { beforeStart, running, afterEnd, halted} // ICO states 
    State public icoState;
    
    constructor(address payable _deposit,address _tokenContract, uint _tokenSupply){
        deposit = _deposit; 
        tokenSupply = _tokenSupply;
        admin = msg.sender; 
        icoState = State.beforeStart;
    tokenPrice = tokenSupply/hardCap;
        tokenContract = ERC20Interface(_tokenContract);

      // 1 ETH = 1000 CRTP, 1 CRPT = 0.001
...
function gettotalWallet()public view returns(uint256){
    return totalWallet;
}
    ...
    function invest() payable public returns(bool){ 
        ...
        uint tokens = msg.value / tokenPrice;
          require(tokenContract.balanceOf(address(this))>= tokens*10**18,"No more token in the contract!");
